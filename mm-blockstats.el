...
(progn
  ...
  (defun mm-blockstats-main (arg-from arg-to arg-report-progress arg-verbose-p arg-namecoin-cli-string)
    ...
    (let (loc-from
	  ...
	  loc-txsplit-step2
	  loc-txsplit-step3
	  loc-parentheadersplit-step0
	  loc-parentheadersplit-step1
	  loc-parentheader
	  loc-parentbitsle
	  loc-parentbits)
      ...
      (loop for i from loc-from to loc-to do
	    ...
	    (setq loc-parentheadersplit-step0 (split-string loc-block "\"parentblock\":"))
	    (setq loc-parentheadersplit-step1 (split-string (cadr loc-parentheadersplit-step0) "\""))
	    (setq loc-parentheader (cadr loc-parentheadersplit-step1))
	    (setq loc-parentbitsle (substring loc-parentheader -16 -8))
	    (setq loc-parentbits (concat (substring loc-parentbitsle -2) (substring loc-parentbitsle -4 -3) "xxxxx" ))
	    ;;(princ (format "%s\n" loc-parentbits))
		...
		(setq loc-resultsymbol (intern (concat loc-parentbits "    DiscusFish-" (format "%05x" loc-coinbaseversion)))))
	       ((string= "c825a1ecf2a6830c4401620c3a16f1995057c2ab" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    F2Pool-"  (format "%05x" loc-coinbaseversion)))))
	       ((string= "80ad90d403581fa3bf46086a91b2d9d4125db6c1" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    ghash.io-"  (format "%05x" loc-coinbaseversion)))))
	       ((string-match "slush" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    slush-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "Eligius" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    Eligius-" (format "%05x" loc-coinbaseversion)))))

	       ((string= "2bf64e2be04ba49a87d17ae9a9009d72efbc4724" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    myBTCcoin-" (format "%05x" loc-coinbaseversion)))))
               ((string= "cef3550ff9e637ddd120717d43fc21f8a563caf8" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    Bixin-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "bitparking" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    bitparking-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "mmpool" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    mmpool-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "bitminter" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    bitminter-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "AntPool" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    Antpool-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "TangPool" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    TangPool-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "give-me-coins" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    give-me-coins-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "TBDice" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    TBDice-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "huobi" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    huobi-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "Guild" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    BTC_Guild-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "/BTCC/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    BTCC-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "/mined by gbminers/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    GBMiners-" (format "%05x" loc-coinbaseversion)))))
	       ((string= "2c30a6aaac6d96687291475d7d52f4b469f665a6" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    BTCC-" (format "%05x" loc-coinbaseversion)))))
	       ((string= "adbc0b208ba2c41fbe9599942cba3c205fe37ed7" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    p2p-1GqdAgC1zmzLFMg3XyYctSECWUjkbT7Nv9-"  (format "%05x" loc-coinbaseversion)))))
	       ((string= "20dfa394c831d087c5695188410b15bda888d903" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    p2p-13zpXrBUF4nYoqBvT2RdbJiNUxH5PBuP74-"  (format "%05x" loc-coinbaseversion)))))
	       ((string= "24e469e9b8c9b3487378717d357c19f8ed062fa7" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    p2p-14N4xNSDtNYatfCgHhyRaAhdZtaczUBnAa-" (format "%05x" loc-coinbaseversion)))))
	       ((string= "159bb0eb02cb1c35f5d4942420d23fff8b6ae62d" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    p2p-12yFhdpG3TxASryDGzWZ268zWQ3hcj2JNu-" (format "%05x" loc-coinbaseversion)))))
	       ((string= "ed3104267d771aeaf7fe9a7fd5dc56f2dc18ad8f" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    p2p-1Nd9sArhgzgLwhaYqoi7Pnm6Jcv5Z58Ghm-" (format "%05x" loc-coinbaseversion)))))
	       ((string= "298d57f3f98fe1121dafcee43bf8cdfed11f2444" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    p2p-14ni2b7mEnvAoXYf6hzxv7wUFAt1TCoavq-" (format "%05x" loc-coinbaseversion)))))
	       ((string= "df5b7a824ee01515c8d8a2bf6813a93aeef10b36" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    p2p-1MN1GpXDADiG1ioGbgz8Tyim2MeBcTrb38-" (format "%05x" loc-coinbaseversion)))))

	       ((string= "27e4efc33ade08c78b85e1bb3931f696271277aa" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    multipool.us-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "StratumPool" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    StratumPool-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "nodeStratum" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    nodeStratum-" (format "%05x" loc-coinbaseversion)))))

	       ((string-match "EMC" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    EclipseMC-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "/ViaBTC/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    ViaBTC-" (format "%05x" loc-coinbaseversion)))))
               ((string-match "/BTC.COM/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    BTC.com-" (format "%05x" loc-coinbaseversion)))))
               ((string-match "/BTC.TOP/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    BTC.TOP-" (format "%05x" loc-coinbaseversion)))))
               ((string-match "/BitClub Network/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    BitClub Network-" (format "%05x" loc-coinbaseversion)))))
               ((string= "2a09f007d28aff030eb6aaeafbfbd2a817d4be34" loc-hash160)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    Allegedly Poolin-" (format "%05x" loc-coinbaseversion)))))
               ((string-match "/poolin.com/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    Poolin-" (format "%05x" loc-coinbaseversion)))))
               ((string-match "/Bixin/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    Bixin-" (format "%05x" loc-coinbaseversion)))))
               ((string-match "/prohashing.com" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    Purportedly prohashing.com-" (format "%05x" loc-coinbaseversion)))))
               ((string-match "/www.okpool.top/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    Purportedly www.okpool.top-" (format "%05x" loc-coinbaseversion)))))
               ((string-match "/E2M & BTC.TOP/" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    Unknown (Coinbase /E2M & BTC.TOP/)-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "P2SH" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    unknown-P2SH-supporter-" (format "%05x" loc-coinbaseversion)))))
	       ((string-match "[BSV]" loc-resultstring)
		(setq loc-resultsymbol (intern (concat loc-parentbits "    unknown-BSV-pool-" (format "%05x" loc-coinbaseversion)))))
	       (t
		(setq loc-resultsymbol (intern (concat loc-parentbits "    unknown-" (format "%05x" loc-coinbaseversion))))))
      ...
      (princ (format "  Total     %%  Parentbits  Pool or Miner, Blockversion"))
      (terpri)
